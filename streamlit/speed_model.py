...
pr_names1 = list(statcan.boundary("provinces").loc[:, "PRNAME"].unique())
pr_names2 = list(statcan.boundary("provinces").loc[:, "PRNAME"].unique())
...
# @st.experimental_memo
def generate_map(prname, mapname="ookla_50_10_percentile", cdname="All", ername="All"):
...
map = generate_map(pr_option,"ookla_50_10_percentile", "All", er_option)
if pin_location:
...
pr_option1 = st.selectbox("Province", pr_names1, index=pr_names1.index("Alberta"),key='11')
er_options1 = ["All"]
er_options1.extend(
    statcan.boundary("economic_regions").loc[lambda s: s.PRNAME == pr_option1, "ERNAME"]
)
er_option1 = st.selectbox("Economic Regions", er_options1,key='12')#

pin_location1 = None

f"Province selected: {pr_option1}"
f"Economic region selected: {er_option1}"
...
addy_lookup = st.text_input("Address Search",key='13')
f"Typed Address: {addy_lookup if addy_lookup.strip() != '' else 'No Address Submitted'}"
if addy_lookup.strip() != "":
    search_result = lookup_address(addy_lookup)
    st.write(search_result)
    ...
    if search_result:
        pin_location1 = search_result[0]["lat"], search_result[0]["lon"]
...
speed_data_subset = subset_region(pr_option1, ername=er_option1)
...
map1 = generate_map(pr_option1,"Down_50_percentile", "All", er_option1)
if pin_location1:
    pin = folium.Marker(pin_location1, popup="Search Location")
    pin.add_to(map1)

st_folium(map1, key="main-map1", returned_objects=[], height=800, width=700)
...
pr_option2 = st.selectbox("Province", pr_names2, index=pr_names2.index("Alberta"),key='21')
er_options2 = ["All"]
er_options2.extend(
    statcan.boundary("economic_regions").loc[lambda s: s.PRNAME == pr_option2, "ERNAME"]
)
er_option2 = st.selectbox("Economic Regions", er_options2,key='22')#

pin_location2 = None

f"Province selected: {pr_option2}"
f"Economic region selected: {er_option2}"
...
addy_lookup = st.text_input("Address Search",key='23')
f"Typed Address: {addy_lookup if addy_lookup.strip() != '' else 'No Address Submitted'}"
if addy_lookup.strip() != "":
    search_result = lookup_address(addy_lookup)
    st.write(search_result)
    ...
    if search_result:
        pin_location2 = search_result[0]["lat"], search_result[0]["lon"]
...
speed_data_subset = subset_region(pr_option2, ername=er_option2)
...
map2 = generate_map(pr_option2,"Up_10_percentile", "All", er_option2)
if pin_location2:
    pin = folium.Marker(pin_location2, popup="Search Location")
    pin.add_to(map2)

st_folium(map2, key="main-map2", returned_objects=[], height=800, width=700)
...
# """
# )
# pr_option1 = st.selectbox("Province", pr_names, index=pr_names.index("Alberta"))
# er_options = ["All"]
# er_options.extend(
#     statcan.boundary("economic_regions").loc[lambda s: s.PRNAME == pr_option1, "ERNAME"]
# )
...
# speed_data_subset = subset_region(pr_option1, ername=er_option)
...
# map1 = generate_map(pr_option1,"ookla_50_10_percentile", "All", er_option)
# if pin_location:
#     pin = folium.Marker(pin_location, popup="Search Location")
#     pin.add_to(map1)

# st_folium(map1, key="main-map", returned_objects=[], height=800, width=700)

# map2 = generate_map(pr_option,"ookla_50_10_percentile", "All", er_option)
# if pin_location:
#     pin = folium.Marker(pin_location, popup="Search Location")
#     pin.add_to(map2)
