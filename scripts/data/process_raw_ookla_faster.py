...
pat = re.compile('.*/type=(?P<type>fixed|mobile)/year=(?P<year>[0-9]{4})/quarter=(?P<quarter>[1-4])/.*')
...
for raw_file in raw_files:
    ...
    filtered_filename = f"ookla-canada-{raw_file.year}-Q{raw_file.quarter}-{raw_file.type}"
    ...
    unknown_tiles.loc[lambda s:s.quadkey.isin(inner_intersection.quadkey), 'in_canada'] = True
    tile_labels = pd.concat([
        tile_labels,
        unknown_tiles[['quadkey','geometry','in_canada']] 
    ],axis=0)
    tile_labels.drop_duplicates(subset=['quadkey'],inplace=True)


    only_canada = a_tile.loc[lambda s:s.quadkey.isin(tile_labels.loc[lambda s:s.in_canada].quadkey)]
